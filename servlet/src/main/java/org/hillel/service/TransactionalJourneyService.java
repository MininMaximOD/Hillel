...
public class TransactionalJourneyService{
    ...
    @Transactional(readOnly = true)
    public Page<JourneyEntity> findAllWithPagination(Pageable page){
        return journeyRepository.findAll(page);
    }

    @Transactional(readOnly = true)
    public Page<JourneyEntity> findAllWithPagination(Pageable page, HashMap<String, String> specifications){
        return journeyRepository.findAll(filter.builderSpecification(specifications), page);
    }
